<template>
  <BaseContainerSecondaryColumn>
    ...
    <p v-if="$fetchState.pending">Fetching inventory...</p>
    ...
    <div v-else>
      ...
      >
        <ItemCardInventory
          ...
          :key="item.row_id"
          :title="item.sublot_no"
          :subtitle="item.item_id"
          :storageLocation="item.storageEnt.ent_name"
          :qrValue="item.sublot_no"
          :isPinned="
            pinnedItems.findIndex(
              x => x.title == item.sublot_no && x.type == 'inventory'
            ) != -1
          "
          :isSelected="item.row_id == currentItem.row_id"
    ...
    <template #aside>
      <InventoryDetailsAside :inventoryItem="currentItem" />
    </template>
  </BaseContainerSecondaryColumn>
</template>
...
export default {
  ...
  async fetch() {
    this.inventory = await fetch(
      `${this.$config.baseURL}/api/inventory`
    ).then(res => res.json());
  ...
  methods: {
    async pinItem(item) {
      ...
      if (
        !storageObj ||
        storageObj.findIndex(x => x.title == pinItem.title) === -1
      ) {
        storageObj.push(pinItem);
      } else {
        storageObj = storageObj.filter(x => x.title != pinItem.title);
      }
      // write updated items
